...
class FluxModDiffusionLoader:
	...
	def load_unet(self, *, unet_name, quant_mode, guidance_name=None,lite_patch_unet_name=None):
		dtypes = {
			"bf16": torch.bfloat16, 
		...
		unet_path = folder_paths.get_full_path("unet_gguf" if is_gguf else "diffusion_models", unet_name)
		...
		if lite_patch_unet_name is not None:
			lite_patch_unet_name = folder_paths.get_full_path("diffusion_models", lite_patch_unet_name)
